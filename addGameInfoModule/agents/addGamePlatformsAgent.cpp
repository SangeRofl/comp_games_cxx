...
#include "addGamePlatformsAgent.hpp"
...
SC_AGENT_IMPLEMENTATION(addGamePlatformsAgent)
    {
        if (!edgeAddr.IsValid())
        ...
        while(it5_1->Next())
        {
            ...
            while(it3_1->Next())
            {
                ...
                while(it5_2->Next())
                {
                    ...
                    std::string GamePlatforms_str;
                    if(value == "gog")
                    {
                        ScLink url_link(*ms_context, url);
                        std::string const text_url = link.Get<std::string>();
                        system(("python3 ../problem-solver/cxx/addGameInfoModule/agents/py_utils/gog.py -agent=GamePlatforms -site="+val).c_str());
                        std::ifstream in("../problem-solver/cxx/addGameInfoModule/agents/py_utils/data/platforms.txt");
                        if (in.is_open())
                        {
                            getline(in, GamePlatforms_str);
                        }
                        in.close();
                    }
                    else
                    ...
                    ScAddr GamePlatforms = ms_context->CreateNode(ScType::NodeConst);
                    CommonUtils::setMainIdtf(ms_context.get(), GamePlatforms, GamePlatforms_str, {Keynodes::lang_ru});
                    ScAddr GamePlatformsText_edge = ms_context->CreateEdge(ScType::EdgeDCommonConst, game, GamePlatforms);
                    ScAddr GamePlatformsText_edgeAccess=ms_context->CreateEdge(ScType::EdgeAccessConstPosPerm, Keynodes::nrel_platform, GamePlatformsText_edge);
                    ms_context->CreateEdge(ScType::EdgeAccessConstPosPerm, answer, game);
                    ms_context->CreateEdge(ScType::EdgeAccessConstPosPerm, answer, GamePlatforms);
                    ms_context->CreateEdge(ScType::EdgeAccessConstPosPerm, answer, Keynodes::nrel_platform);
                    ms_context->CreateEdge(ScType::EdgeAccessConstPosPerm, answer, GamePlatformsText_edge);
                    ms_context->CreateEdge(ScType::EdgeAccessConstPosPerm, answer, GamePlatformsText_edgeAccess);
                    break;
                }
            }
        }
